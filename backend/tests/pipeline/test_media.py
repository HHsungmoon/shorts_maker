...
DB 는 비운 테스트 Postgres(`self.conn`, tests/support.DbTestCase), 파일은 임시 디렉터리.
"""
...
class MediaTestCase(DbTestCase):
    def setUp(self) -> None:
        super().setUp()
    ...
    def register(self, path: Path) -> int:
        # fingerprint 는 unique — 파일명으로 만들어 등록마다 다르게 한다.
        row = self.conn.execute(
            """insert into sources (title, content_type, path, duration_sec, fingerprint)
               values ('강연', 'LECTURE', %s, 1800, %s) returning id""",
...
class DeleteTest(MediaTestCase):
    ...
    def test_removes_derived_files_and_rows(self):
        ...
        chunk_id = self.conn.execute(
            "insert into chunks (source_id, idx, start_sec, end_sec, path) values (%s, 0, 0, 1800, %s) returning id",
        ...
        segment_id = self.conn.execute(
            """insert into segments (chunk_id, idx, start_sec, end_sec,
               ...
               values (%s, 0, 0, 300, 0, 5) returning id""",
            (chunk_id,),
        ).fetchone()["id"]
        ...
        self.conn.execute(
            "insert into clips (run_id, segment_id, start_sec, end_sec, path) values (%s, %s, 0, 45, %s)",
    ...
    def test_never_touches_files_outside_the_managed_directories(self):
        ...
        source_id = self.register(path)
        self.conn.execute(
            "insert into chunks (source_id, idx, start_sec, end_sec, path) values (%s, 0, 0, 10, %s)",
