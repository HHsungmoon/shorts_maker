...
from shorts_maker import ffmpeg, ingest
...
class _IngestDbCase(unittest.TestCase):
    """원본 파일과 빈 DB 를 갖춘 케이스. ffmpeg 는 부르지 않는다 — 여기서 보는 건 행의 상태다."""
...
class ChunkReplaceTest(_IngestDbCase):
    ...
    def _grow_a_tree(self, chunk_id: int) -> Path:
        ...
        self.conn.execute(
            "insert into utterances (chunk_id, idx, start_sec, end_sec, text) values (?, 0, 0, 5, '안녕')",
            (chunk_id,),
        )
        ...
        run_id = self.conn.execute(
            "insert into runs (source_id, status, ranked) values (?, 'DONE', '{}')", (self.source_id,)
        ).lastrowid
        ...
        self.conn.execute(
            "insert into clips (run_id, segment_id, start_sec, end_sec, path, rendered) values (?, ?, 0, 5, ?, 1)",
    ...
    def test_replace_keeps_the_idx_and_removes_everything_below(self):
        ...
        row = dict(self.conn.execute("select * from chunks where id = ?", (second,)).fetchone())
