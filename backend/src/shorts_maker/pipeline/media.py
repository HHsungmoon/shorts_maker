...
def listing(conn: psycopg.Connection, cfg: config.Config) -> list[dict]:
...
def derived_paths(conn: psycopg.Connection, cfg: config.Config, source_id: int) -> list[Path]:
    ...
    """
    paths: list[Path] = []
    for row in conn.execute("select path from chunks where source_id = %s", (source_id,)):
        if row["path"]:
...
def find_source_id(conn: psycopg.Connection, cfg: config.Config, path: Path) -> int | None:
    """정규화한 경로로 대조한다.
...
def delete(conn: psycopg.Connection, cfg: config.Config, name: str) -> Removal:
