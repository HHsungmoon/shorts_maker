...
# 🔴 6번까지 기다린다(2·4·8·16·32초 = 최대 62초).
#
# 4번(=14초)이었는데 2026-09-09 에 모자랐다. "This model is currently experiencing high demand"
# 503 이 몇 분씩 이어졌고, 답하기 한 번이 428초 걸린 것도 대부분 이 재시도였다. 14초 만에
# 포기하면 그때까지 쓴 호출이 통째로 버려진다 — 무료 등급에서는 그게 곧 하루 할당량이다.
#
# 위로 더 늘리지 않는 이유: 잡 워커가 하나라(jobs.py) 한 호출이 붙잡고 있으면 큐 전체가 멈춘다.
MAX_ATTEMPTS = 6
