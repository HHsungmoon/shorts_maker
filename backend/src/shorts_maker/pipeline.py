...
from . import config, cutting, ranking, render, segmentation
...
def run_all(
    ...
    chunks = conn.execute(
        "select id from chunks where source_id = ? order by idx", (source_id,)
    ).fetchall()
    if not chunks:
    ...
    for chunk in chunks:
        existing = conn.execute(
            "select count(*) as n from segments where chunk_id = ?", (chunk["id"],)
        ).fetchone()["n"]
