...
def load_utterances(conn: sqlite3.Connection, chunk_id: int) -> list[dict]:
    rows = conn.execute(
        "select idx, start_sec, end_sec, text from utterances where chunk_id = ? order by idx",
        (chunk_id,),
    ).fetchall()
...
def run_for_chunk(
    conn: sqlite3.Connection, cfg: config.Config, chunk_id: int, force: bool
) -> list[SegmentSpec]:
    chunk = conn.execute(
    ...
    existing = conn.execute(
        "select count(*) as n from segments where chunk_id = ?", (chunk_id,)
    ).fetchone()["n"]
    ...
    call = conn.execute(
        """insert into stage_calls
           (source_id, stage, model, input_tokens, output_tokens, thinking_tokens, latency_ms, params)
           values (?, 'segment', ?, ?, ?, ?, ?, ?)""",
        (
            chunk["source_id"],
            cfg.gemini_model,
            ...
            latency_ms,
            json.dumps({"utterances": len(utterances)}, ensure_ascii=False),
        ),
    )
    conn.commit()
    ...
    conn.execute("delete from segments where chunk_id = ?", (chunk_id,))
