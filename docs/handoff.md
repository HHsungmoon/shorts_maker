...
**2026-09-07 (9차): 루프 닫기(M6).** M5 까지 백엔드는 다 됐는데 화면이 비어 있었다 — 데모 8단계 중
...
**2026-09-09 (12차): 원본 유입 경로.** 감사에서 1순위로 나온 것을 만들었다 — 이 제품의 핵심 주장이
"숏폼을 보고 원본으로 들어간다" 인데 구현이 명세와 **반대**였다(새 탭으로 유튜브를 열고, 초조차 없었다).
유튜브 IFrame Player API 를 붙여 숏폼 CTA 가 같은 페이지의 플레이어를 그 초로 옮긴다.
같이 고친 계측 셋: `clip_id` 가 한 줄도 안 채워지던 것(프론트는 payload 에 넣고 서버는 최상위를 읽었다),
좋아요가 `source_id` 를 null 로 넣던 것, 공개 이벤트 라우트가 없는 id 에 500 을 내던 것.
그리고 `viewer_events` 를 **처음으로 읽는 코드**가 생겼다 — 기록 탭의 퍼널 표(M6b).

**2026-09-09 (13차): 구간 검색을 LLM 으로 바꿨다.** 답변 경로가 왜 답을 못 하는지 파고들었고
**가설이 두 번 바뀌었다.** 처음엔 판정이 빡빡한 줄 알고 프롬프트를 완화했는데 결과가 안 바뀌었다.
검색 결과를 열어 보니 답이 담긴 구간이 44개 중 **29·30위**였다 — 모델은 받은 대사만 보고 정직하게
거절한 것이었다. 1위는 질문의 "기술" 과 글자가 겹치는 "기술 문화" 였다.

그래서 고르는 주체를 임베딩에서 LLM 으로 바꿨다. 구간 **설명 전부**를 라우팅 호출에 얹어
경로 판정과 구간 선택을 한 번에 한다 — **호출 수가 늘지 않는다**(답변당 5회 유지). 근거와 대안
비교는 update_plan M5 에 있다. 임베딩은 다른 편 안내(상대 비교)와 폴백에만 남는다.

같이 잡은 것 둘. **분당 한도를 일일 소진으로 오분류하던 버그** — 무료 등급의 모든 한도 메시지에
들어 있는 조각을 표식으로 쓰고 있었다. 데모에서 터질 버그였다. 그리고 다른 편 안내가
**조용히 퇴화하던 것** — 형제 영상이 인덱싱돼 있지 않으면 일반 문구가 떴다.

🔴 여전히 **[답하기]가 실데이터로 클립을 만든 적이 없다.** 위 변경이 실제로 답을 만드는지는
Gemini 할당량이 풀린 뒤 확인해야 한다.

🔴 **Gemini 무료 등급 하루 20회에 걸렸다**(2026-09-06). 모델당 따로이므로 `SHORTS_GEMINI_MODEL` 을
바꾸면 즉시 풀리고, 계속 쓸 거면 결제를 붙여야 한다. 코드는 두 가지를 고쳤다 —
일일 할당량은 재시도하지 않고(분당 한도와 구분), 주제 분할은 끝난 조각을 건너뛴다.
...
**2026-09-09 에 1~5번을 만들었다**(원본 유입 경로 + 계측 + insights). 아래는 그 뒤 남은 것이다.

| # | 남은 것 | 왜 막히나 |
|---|---|---|
| 1 | 답변 경로 평가 세트 (`eval-answer`) | `eval/questions.json` 은 `group` 만 있고 `type`·`expect` 가 없다. `sm tease` 명령군 자체가 없다. **라우팅·best-of-3·조합·UNANSWERABLE 이 질문 세트로 측정된 적이 없다** — 데모 당일이 첫 시험이 된다 |
...
이벤트 7종이 전부 발생하고 `clip_id` 가 채워진다. `GET /api/sources/{id}/insights` 와 기록 탭의 퍼널 표.
공개 이벤트 라우트가 id 를 검증한다(없는 id → 500 이 아니라 404).
🔴 **브라우저에서 눈으로 확인한 것은 아니다** — 재빌드 후 확인이 남았다.
...
- **Gemini 무료 등급은 모델당 하루 수십 회다.** 답하기 한 번에 LLM 4회(rank 1 + judge 3)를 쓴다.
  할당량이 떨어지면 `gemini.is_daily_quota` 가 재시도 없이 무엇을 하면 되는지 알려준다.
- **주제 분할은 끝난 조각을 건너뛴다.** 다만 중간이 비면 번호를 이어 붙일 수 없어 그 뒤는 다시 만든다.
