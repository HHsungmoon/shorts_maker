...
- [ ] 🔴 **유튜브 IFrame Player API + 숏폼 끝 CTA → `player.seekTo()`.** 지금은 `enablejsapi` 없는 평범한
      `<iframe>` 이고 CTA 는 `target="_blank"` 로 **유튜브를 새 탭에 연다 — `t=` 초조차 없다.**
      명세된 동작(페이지를 떠나지 않고 그 초로 이동)의 반대다. `youtube-nocookie.com` 도 안 쓴다.
      선행 조건: `/api/watch/sources/{id}` 의 클립에 `start_sec` 이 없어서 이동할 초를 프론트가 모른다
- [ ] 계측: `short_play` ✅ · `short_complete` ✅ · `cta_click` ⚠️(외부 링크에 붙어 있다) ·
      `question_post` ✅ · `like` ⚠️(`source_id` 를 null 로 넣어 영상별 집계가 불가능하다) ·
      `origin_seek` ❌ · `origin_play` ❌ (뒤 둘은 위 항목이 선행)
      🔴 **`viewer_events.clip_id` 가 한 번도 채워지지 않는다** — 프론트가 클립 id 를 `payload` 안에 넣고
      서버는 top-level `clipId` 를 읽는다. `idx_events_clip` 은 죽은 인덱스고 클립별 퍼널이 불가능하다
...
유튜브 플레이어가 그 초로 이동한다 ❌. 이벤트 7종이 `viewer_events` 에 쌓인다 → **5종만, 그중 2종은 결함**.
...
### M6b — insights (S) — 컷 라인 밖

- [ ] `GET /api/sources/{id}/insights` — 퍼널(재생·완주·CTA·원본 이동)을 클러스터별로
- [ ] 스튜디오에 표 하나. 🔴 숫자가 적을 때 비율을 쓰지 않는다 — 3명 중 1명이 33% 로 보이면 거짓말이다
