...
6. **이름.** 패키지·레포·CLI 는 `shorts_maker`/`sm` 그대로 둔다. **CLIPQ** 는 제품 이름이라 화면 문자열과
   제출 문서에만 쓴다(`web/src/shared/brand.ts` 상수 하나). 해커톤 중에 레포를 개명하지 않는다.
   🔴 2026-09-16 에 화면 이름을 TEASE → **CLIPQ** 로 바꿨다(D14). 도메인·기획서와 같은 이름으로 모았다.
...
sequenceDiagram
  ...
  W->>W: 익명 쿠키 확인·레이트리밋
  W->>E: 발행 숏폼 추천 — 임베딩 최대 1회(D13 조건)
  W-->>V: {questionId, suggestions}  (cluster_id null)
  V->>W: POST events {short_play|cta_click|origin_seek|origin_play}
  Note over E,C: 묶기는 여기서 돌지 않는다 — 크리에이터의 [집계](§2-2) 에서
```

**공개 경로의 LLM 호출은 0회다.** 외부 호출은 질문 등록의 숏폼 추천 임베딩 최대 1회뿐이고 조건이 붙는다(D13). 묶기는 크리에이터가 트리거한다(D11).
...
backend/src/shorts_maker/
  ...
  answers/                                               시청자 질문 → 답 클립. 제품명(CLIPQ)은 코드에 안 쓴다
...
밖에 있는 것의 기준"을 아무도 설명 못 한다 — CLIPQ 는 부가 기능이 아니라 제품이다. HTTP · 파이프라인 · 답변 ·
...
- [x] `web/src/shared/brand.ts`: `PRODUCT_NAME = "CLIPQ"`, `REPO_NAME = "shorts_maker"`. 제목 2곳 + index.html
...
- [x] 🔴 `useAsync` 의 "로딩 중 data=null 깜빡임" — 완료 2026-09-09. `keepPrevious` 옵션을 넣고
      **폴링하는 스튜디오 화면**에 켰다(홈·영상 페이지·퍼널). 잡이 도는 동안 `reloadToken` 이
      올라가며 통째로 다시 읽는데, 그때마다 패널이 빈 화면이 됐다.
      🔴 `subject` 를 함께 둔 이유: 대상이 바뀌면(다른 영상) 유지하지 않고 비운다 — 깜빡임을
      없애려다 **옛 영상의 내용을 잠깐 보여주는** 것은 빈 화면보다 나쁘다.
      🔴 `loading` 의 뜻을 "요청 중" 에서 **"그릴 것이 없다"** 로 바꿨다. 그래서 기존 호출부의
      `if (loading)` 가드가 저절로 옳게 동작한다. 뒤에서 다시 읽는 중은 `reloading` 이다.
      곁들여 고친 것: 폴링 중 한 번 실패했다고 화면을 비우지 않는다. 대신 오류 문구를 둘로 갈랐다 —
      내용이 살아 있는데 "영상을 찾을 수 없습니다" 가 뜨면 그건 거짓말이다
...
#### 🔴 M5 실측 — 구간 검색이 이 질문에 안 듣는다 (2026-09-09)

"쏘카 개발 직군의 주요 기술 스택은?" 이 답변 불가로 끝난 것을 파고들었다. **처음 세운 가설은
틀렸다.** 판정이 빡빡한 게 원인이라고 봤는데, 프롬프트를 완화한 뒤 다시 부르니 같은 이유로
거절했다(generate_content 1회). 그래서 검색 결과를 열어 봤다.

44개 구간 전체를 질문 벡터와 재본 결과(캐시된 임베딩, 호출 0회):

| 순위 | 구간 | 유사도 | 설명 |
|---|---|---|---|
| 1 | [4] | 0.755 | 쏘카의 **기술 문화** — 성장 지원·협업 |
| 2 | [2] | 0.739 | 차량 제어·데이터 수집·개인화 |
| 3 | [1] | 0.729 | CTO 인사, 발표 안내 |
| 4 | [40] | 0.712 | 기술 조직의 미래 방향성 |
| 5 | [13] | 0.710 | 프로덕트 세 가지 유형 |
| **29** | **[7]** | **0.599** | **마이크로서비스 아키텍처 도입** ← 실제 답 |
| **30** | **[22]** | **0.597** | **딥러닝 AI 프로젝트 · MLOps 플랫폼** ← 실제 답 |
| 44 | [36] | 0.502 | 발언 정리·감사 |

**읽히는 것 셋:**

1. **답이 있는 구간이 29·30위다.** top_k 를 늘려서 될 일이 아니다 — 29위를 넣으려면 44개 중
   30개를 보내야 하고, 그건 검색을 안 하는 것과 같다(구간당 약 870 입력 토큰 × 30).
2. **전체가 0.502~0.755 의 좁은 띠에 몰려 있다.** 그래서 `SHORTS_RETRIEVAL_MIN_SIM=0.5` 는
   **한 번도 아무것도 거르지 않는다.** "이 영상엔 그 얘기가 없다를 rank 전에 안다"(§ 이 파일 M5
   설계 근거)는 지금 동작하지 않는다. 복지·인재상 질문이 0.74~0.755 로 통과한 것도 이 때문이다.
3. **표면 낱말이 이긴다.** 1위 "기술 문화" 는 질문의 "기술" 과 글자가 겹치고, 실제 답인
   "마이크로서비스 아키텍처" 는 질문의 낱말과 하나도 안 겹친다. 짧은 한국어 **구간 설명**을
   임베딩하는 것으로는 구체적 기술 내용과 일반적 조직 얘기가 갈리지 않는다.

**즉 이건 판정 문제가 아니라 검색 문제다.** 판정을 아무리 후하게 해도 입력에 답이 없으면 답할 수 없다.
프롬프트 완화(`ranking.ANSWER_PROMPT`·`judge.PROMPT`)는 한쪽으로 치우친 눈금을 고친 것이라 그대로
두지만, 이 건의 해결책은 아니다.

- [x] 🔴 **검색 단계를 다시 설계했다 — ①번 (2026-09-09).** 구간 설명 **전부**를 LLM 에게 주고
      번호로 고르게 한다. 셋 중 ①을 고른 이유:
      - ③(전부 다 넘기기)은 위험을 옮기기만 한다. 다음 단계는 번호 붙은 대사에서 시작·끝점을
        정확히 골라야 하고, 구간 8개면 약 300줄인데 44개 전부면 1689줄이다 — 검색 실패가
        **정밀도 실패**로 바뀐다
      - ②(대사 임베딩)는 될지 모른다. 재보려다 분당 한도에 걸렸고, 짧은 질문으로 긴 대사를 찾는
        비대칭은 그대로 남는다
      - ①은 **이미 이 레포에서 되는 방식**이다 — 기준 rank 경로가 설명 전부를 놓고 고른다
      🔴 **호출 수가 늘지 않는다.** 라우팅이 이미 LLM 을 한 번 쓰면서 질문 문장만 보내고 있었다 —
      같은 호출에 구간 목록을 얹어 경로 판정과 구간 선택을 한 번에 한다(`routing.decide`).
      답변당 5회 유지. ⚠️ 한 호출에 두 일을 시킨 대가는 `stage_calls.params` 로 재기로 했다
- [x] `SHORTS_RETRIEVAL_MIN_SIM` 의 자리를 옮겼다 — "이 영상엔 없다" 판정은 이제 LLM 이 한다
      (빈 배열). 그 설정값은 **폴백 경로에만** 남는다(판정을 못 읽었을 때의 임베딩 top-k).
      임베딩은 다른 편 안내에도 계속 쓴다 — 거기는 **절대 임계값이 아니라 이 영상 최고점과의
      상대 비교**다. 0.50~0.755 좁은 띠에서는 그것만이 뜻이 있다
- [x] 🔴 **분당 한도를 일일 소진으로 오분류하던 버그를 고쳤다** (2026-09-09, 위 측정 도중 발견).
      표식에 `free_tier_requests` 가 있었는데 무료 등급의 **모든** 한도 메시지에 그 조각이 들어 있어서
      분당 한도까지 "오늘은 안 풀린다" 로 죽었다. 서버가 준 `retryDelay`(36초)도 무시하고 2·4·8초만
      기다리다 실패했다. 판단은 quotaId 의 `PerDay`/`PerMinute` 로 하고, 대기는 서버 값을 따른다
      (상한 65초 — 워커가 하나뿐인 큐가 막히면 안 된다). **데모에서 터질 버그였다**: 두 번째 영상을
      인덱싱하면 분당 한도에 쉽게 닿는다(한도가 호출 수가 아니라 텍스트 개수로 세지는 것으로 보인다)
#### 🔴 M5 실측 2 — 첫 실데이터 관통과 그 뒤 (2026-09-09~10)

**관통했다.** "개발 직군의 주요 기술 스택" 이 답변 불가에서 답이 됐고(검색 재설계의 효과),
조합 클립이 실제로 렌더됐다 — 18.93초, 1080×1920, 브릿지 카드에 "45:26에서 이어집니다" 정상 표시.
`clip_parts` 가 처음으로 2행이 됐다.

**그런데 후보 셋이 전부 자립성 판정에서 떨어졌다.** 이유가 같았다 —
"저희는 **이런 서비스들**을 …", "**이렇게** 수많은 AI 프로젝트들이 …". 검색도 rank 도 맞는 대사를
골랐는데 **시작점이 한 발화 늦었다.** 프롬프트에는 이미 "지시대명사로 시작하지 않는다" 가 있었다.

원인을 파 보니 **rank 가 더 앞에서 시작하고 싶어도 그럴 수가 없었다** — 선행사가 있는 발화는
앞 구간의 것이라 프롬프트에 아예 없었다. 구간은 주제 경계일 뿐이고 사람 말은 거기서 끊기지 않는다.

- [x] `number_lines` 에 **어깨**(`LEAD_IN_SHOULDER=2`) — 구간 머리 앞 발화를 함께 보여준다
- [x] `cutting.lead_in` — 그래도 앞을 가리키며 시작하면 **코드가 시작점을 당긴다**.
      🔴 착지 자리는 "지시어 없음" 만으로 부족하다. 실측에서 지시어를 피했더니 "해나가고 있고"
      에서 시작했다 — 문장 한복판이라 더 나빴다. 그래서 **앞 발화가 문장을 끝냈는지**까지 본다.
      좋은 자리를 못 찾으면 그냥 둔다. 어중간하게 당기느니 판정에 맡긴다
- [x] 예산을 넘기면 당기지 않는다 — 앞머리만 붙고 답이 반이 된 클립보다 나으니까
- [x] 보정 뒤 **같아진 후보는 버린다**(실측: tight 를 당겼더니 single 과 동일해졌다).
      똑같은 카드 둘은 고를 것이 아니고 judge 호출만 하나 더 쓴다

**효과 측정**(같은 질문, 호출 1회): rank 가 **스스로** 어깨에서 시작했고(single), 놓친 것은 보정이
당겼다(tight). 조합의 첫 조각만 예산에 막혀 그대로다 — 당기면 두 번째 조각이 통째로 잘려
조합이 아니게 된다.

- [ ] 위 변경 뒤 **판정까지 다시 통과하는지**는 아직 안 봤다. rank 1회만 확인했다

#### 🔴 운영 실측 — Gemini 503 이 몇 분씩 이어진다 (2026-09-09~10)

답하기 한 번이 **428초** 걸렸고 대부분 "This model is currently experiencing high demand" 재시도였다.
그 뒤 다른 호출은 재시도 4회(총 14초)를 다 쓰고 실패했다.

- [x] `MAX_ATTEMPTS` 를 4 → **6**(2·4·8·16·32초 = 최대 62초). 14초에 포기하면 그때까지 쓴
      호출이 통째로 버려지고, 무료 등급에서 그건 곧 하루 할당량이다. 위로 더 늘리지 않는 이유는
      잡 워커가 하나라 한 호출이 큐 전체를 멈추기 때문이다
- [ ] 🔴 **데모 리스크.** 라이브 단계(시나리오 4·6)가 7분 걸릴 수 있다. M9 리허설에서 실측하고,
      필요하면 그 두 단계는 미리 만들어 둔 것을 보여준다

- [ ] 🔴 **실데이터 관통이 아직 0회다.** `clip_parts` 0행 — 조합 경로는 위 합성 실측이 전부다.
      실DB 의 OPEN 질문("쏘카에서 만드는 주요 프로덕트는 무엇인가요?") 이 첫 시험 대상이다
...
| D9 | 레포·패키지 이름 유지, CLIPQ 는 화면·문서 | 해커톤 중 개명은 위험만 있고 이득이 없다 |
...
| D12 | **결정(2026-09-16).** `/` 는 첫 화면(두 문) · `/watch/**` 시청자 · `/studio/**` 스튜디오 | 도메인을 붙이고 나서 드러났다 — `/` 를 누른 사람이 관리자 로그인 화면을 만났다. 수가 많은 쪽은 시청자이고 심사위원은 스튜디오를 찾아야 하는데, 한쪽을 `/` 에 놓으면 다른 쪽이 숨는다. 그래서 `/` 를 고르는 화면으로 두고 둘을 같은 크기로 보여준다. tease §8-2 를 그렇게 고쳤다 |
| D13 | **질문을 남기는 순간 발행된 숏폼을 추천한다** — 공개 경로에 임베딩 1회를 조건부로 허용(2026-09-15). D11 의 "질문 등록은 insert 만" 을 이 한 곳에서만 푼다. LLM 은 여전히 0회, 묶기는 여전히 [집계] | 이미 답이 나가 있는데 시청자가 그걸 모르고 같은 걸 또 묻는 게 가장 아까운 순간이다. 위험은 둘이었고 둘 다 구조로 막는다. ① **질문 등록이 멈춘다** — 기존 임베딩 함수는 분당 한도에 걸리면 6회까지 최대 65초씩 기다린다 → 재시도 없는 전용 호출 + 제한 시간. ② **도배가 크리에이터 작업의 분당 한도(100)를 먹는다** → 공개 경로 전용 분당 상한 + 비교할 발행 숏폼이 없으면 호출 자체를 안 한다. 질문은 먼저 커밋해 추천 실패가 등록 실패가 되지 않는다. 비교 대상은 **클립의 실제 대사**다 — 실측(질문 8·숏폼 2)에서 제목·구간 설명은 맞는 연결과 틀린 연결의 틈이 음수였고 대사만 +0.015 였다. 묶기용 벡터(SEMANTIC_SIMILARITY, θ 0.85)는 "복지" 를 "기술문화" 숏폼에 0.870 으로 붙여 쓸 수 없었다. 틀릴 수 있어서 "답입니다" 가 아니라 "비슷한 궁금증에 답한 숏폼" 으로 보여주고, 질문을 묶음에 자동으로 붙이지 않는다 |
| D14 | **제품명은 CLIPQ**(2026-09-16). 화면·문서·제출물 전부. 레포·패키지·CLI 는 `shorts_maker`/`sm` 유지(D9 그대로) | 이름이 셋이었다 — 레포 `shorts_maker`, 화면 `TEASE`, 기획서 `CLIPQ`. 도메인을 **clipq.cloud** 로 사면서 어긋나는 쪽이 분명해졌다: 처음 들어온 사람이 주소창에서 읽는 이름과 심사위원이 받는 기획서가 같은데 화면만 달랐다. 바꾸는 값은 `brand.ts` 상수 하나와 `index.html` 의 `<title>` 뿐이다 — 제품명을 코드·DB·환경변수에 넣지 않았기 때문이다(D9 의 실제 효과다). `SHORTS_TEASER_MAX_SEC` 의 teaser 는 제품명이 아니라 예고편이라 그대로 둔다. 문서 파일 `tease.md` 도 링크가 전부 깨지므로 이름을 유지한다 |
| D15 | **여럿이 동시에 쓰는 것을 전제로 바꿨다**(2026-09-16). 로그인 잠금은 IP 별 + 전역 백스톱, 실행은 이미 돌고 있으면 409 | 보기 전용 비밀번호를 화면에 공개하면서 "운영자 1명" 전제가 깨졌다. 코드를 읽어 실제로 깨지는 자리를 둘로 좁혔다. ① 실패 카운터가 프로세스 전역이라 **남의 오타 다섯 번이 관리자까지 60초 잠갔다** — 붙여넣기에 딸린 공백 하나가 실패 한 번이라 심사 중에 반드시 일어난다. IP 별로 세되(nginx 가 덮어쓰는 X-Real-IP) IP 를 돌려 가며 두드리는 쪽은 전역 백스톱이 받는다. ② `submit` 이 무조건 받아 줄을 세웠다 — 워커가 1개라 안전하긴 해도 다섯 명이 누른 다섯 개가 조용한 대기열이 되고 Gemini 하루 할당량이 그만큼 탄다. 거절하고 무엇이 몇 초째 도는지 말해 주는 편이 정직하다. 🔴 검사와 등록을 같은 락 안에 둔다 — 밖에서 active() 로 보고 넣으면 동시에 누른 둘이 둘 다 통과한다. 시청자·보기 전용은 잡을 띄울 수 없어 이 경로와 무관하고, 커넥션 풀 8은 ms 단위 조회라 수십 배 동시 요청을 받아넘긴다 — 풀은 `SHORTS_DB_POOL_MAX` 로 뺐지만 올리려면 db 컨테이너 메모리 상한이 따라와야 한다 |

| D16 | **영상 개요를 AI 가 초안으로 써 준다 — 구간 분할 *뒤*에, 저장은 사람이**(2026-09-19). `POST /api/sources/{id}/overview`, LLM 1회, `stage_calls.stage='overview'` | 개요(프롬프트 3층)는 구간 분할·순위·자르기·후보 생성 **모든 호출에 붙는** 한두 문장인데, 2시간짜리를 처음 받은 사람은 영상을 다 보기 전에는 쓸 수가 없다. 그 답은 이미 `segments.description` 에 쌓여 있다. **순환이 하나 있다**: 개요는 구간 분할 프롬프트에도 들어가므로 분할 결과로 개요를 지으면 그 분할은 개요 없이 된 것이다. 끊는 자리는 둘이었다 — ① 전사 직후(재료가 도입부 발화뿐. 2시간짜리의 앞 5분은 인사와 안내라 후반 주제를 통째로 놓친다. 분할에 반영되지만 틀리면 분할이 더 나빠진다) ② **분할 직후**(재료가 영상 전체. 분할에는 못 쓴다). ②를 골랐다: 개요가 값을 하는 곳은 순위·자르기·답하기이고 거기엔 저장 즉시 반영된다. 분할까지 고치려면 개요를 저장하고 다시 나누면 되는데 전사가 재사용되어 몇 분이고, 개요를 잘못 짚어 전사부터 다시 하는 것보다 싸다. 🔴 **저장하지 않는다** — 틀린 한 줄이 조용히 파이프라인 전체에 퍼지므로 사람이 읽고 누르는 관문을 둔다(후보를 저장하고 멈추는 [답하기]와 같은 이유). 화면은 사람이 이미 손댄 개요를 초안으로 덮지 않는다. 🔴 **관리자 기준(2층)을 넣지 않는다** — 개요는 사실이고, 취향이 섞이면 3층으로 위장해 이후 모든 호출에 따라붙는다(`test_standards.py` 가 막는다). 요약은 **고르게 솎아** 80줄까지만 보낸다 — 21분에 44개였으니 2시간이면 200개가 넘고, 앞에서부터 자르면 후반 주제를 잃어 개요가 틀린다. 잡 큐를 지나는 것도 다른 LLM 호출과 같다 — 호출 하나는 몇 초지만 예외를 두면 그 예외가 다음 예외의 근거가 된다. `describe`(구간 하나하나의 해설, FILM 용)를 재활용하지 않고 마이그레이션 009 로 값을 늘린 이유도 같다: 같은 값으로 적으면 "구간 해설에 얼마 썼나" 가 조용히 부푼다 |
