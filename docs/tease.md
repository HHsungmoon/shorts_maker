# CLIPQ — 시청자 수요로 만드는 숏폼, 원본 유입 엔진

*(파일 이름 `tease.md` 는 옛 제품명의 흔적이다. 제품명은 CLIPQ — §1-1)*
...
CLIPQ 는 그 **기준 프롬프트의 출처를 시청자 질문으로 넓힌 것**이다. 파이프라인 본체는
...
**CLIPQ** — Clip + Question. 시청자의 질문(Q)이 클립을 만든다. 도메인도 clipq.cloud 다.

설계 원칙은 이름이 바뀌어도 그대로다: 완결된 소비가 아니라 **미완의 소비를 설계**해 원본으로
되돌려보낸다. (2026-09-16 이전 이름은 **TEASE** — "살짝 보여주고 애태우다" 였고, 위 원칙은 그
이름에서 나왔다. 이 문서의 파일 이름 `tease.md` 가 그 흔적이다 — 링크가 많아 그대로 둔다. D14)
...
CLIPQ 는 숏폼 생성기이면서 동시에 **콘텐츠 수요 리서치 툴**이다. 장기적으로 팔릴 자산은
...
1. 원티드 영상(21분) 시청 중. 플레이어 아래 CLIPQ 패널.
...
`make_shorts.md` §3 의 원칙이 CLIPQ 의 비용 구조를 이미 만들어 놨다.
...
| | 화면 | 인증 |
|---|---|---|
| `/` | **첫 화면** — 시청자·관리자 두 문 (2026-09-16, D12) | 없음 |
| `/watch` | watch 홈 — 발행된 영상 카드 목록 | 없음 |
| `/watch/:sourceId` | 플레이어 + 질문 패널 + 숏폼 줄 + 질문 직후 추천(D13) | 없음 |
| `/studio` | 영상 목록 | 로그인 |
| `/studio/prompts` | 프롬프트 네 층 · 관리자 기준 | 로그인 |
| `/studio/sources/:id` | 질문 · 새 클립 · 공개 · 영상 준비 · 리포트 · 기록 탭 | 로그인 |

🔴 성과(퍼널)는 별도 라우트가 아니라 영상 페이지의 **기록 탭**이다 — 퍼널은 영상별 값이라
영상 밖에 두면 "어느 영상 것인가" 가 화면에서 사라진다.

로그인은 **역할이 둘**이다. 관리자 비밀번호는 전부, 보기 전용 비밀번호는 읽기만 — 심사처럼 여럿이
들여다볼 때 구경은 열고 실행은 막는다(CLAUDE.md 인증 경계).
