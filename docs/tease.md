...
4. 궁금한 게 목록에 없으면 **직접 적는다** → 비슷한 질문이 이미 있으면 그쪽에 합쳐지고(§5-3)
   "N명이 궁금해함"이 올라간다. 없으면 새 질문. **여기서 AI 는 돌지 않는다** — 요청 등록이다.
...
1. 로그인(비밀번호). 채널 영상 목록.
2. 영상별 **질문 TOP N** — 좋아요·묶인 질문 수 순. "우리 영상에서 사람들이 뭘 궁금해하는지"가
   여기서 보인다.
...
```
                           ┌────────────────────────── /watch (공개) ──────────────────────────┐
  시청자 ── 질문 등록 ──► questions ──► [임베딩] ──► 기존 클러스터와 비교 ──► question_clusters
        ── 좋아요 ─────► question_likes                                          (OPEN)
        ── 숏폼 재생 / CTA / 원본 이동 ──► viewer_events
                           └───────────────────────────────────────────────────────────────────┘
                                                        │
                           ┌────────────────────────── /studio (인증) ─────────────────────────┐
  크리에이터 ── 클러스터 선택 ──► "이 질문에 답하기"
                                    │
...
**(a) 질문 묶기.** 새 질문 → 임베딩(`task_type=RETRIEVAL_QUERY`) → 같은 영상의 기존 클러스터
`canonical_text` 임베딩과 코사인 → 최대값 ≥ θ 면 그 클러스터에 합류, 아니면 새 클러스터.
클러스터가 3개 이상 모이면 LLM 이 대표 문장을 다시 쓴다(개별 질문들을 아우르는 한 문장).
🔴 **θ 가 성패를 가른다.** 너무 높으면 캐시 적중률이 0 으로 수렴해 비용·속도 우위가 동시에
무너지고, 너무 낮으면 다른 질문이 섞인다. 초기값 0.85, §10-3 의 질문 세트로 튜닝 **[미결]**.
...
| `POST /api/watch/sources/{id}/questions` | 질문 등록 → 임베딩·묶기 → `{clusterId, merged: bool, likes}` |
...
| | 설명 |
|---|---|
| `GET  /api/sources/{id}/clusters` | 클러스터 + 개별 질문 펼침 + 좋아요 + 상태 |
...
2. 심사위원이 질문을 적는다 — **"급여 수준이 어떤가요?"** → 기존 "희망연봉 얼마나…" 클러스터에
   **합쳐진다**, "N명이 궁금해함" 이 +1. (질문 묶기, AI 호출은 임베딩 1회)
