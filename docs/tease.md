...
### 5-4. 라우팅 + 구간 선택 — 한 호출로 둘

쿼리 의미가 있는 질문("연봉 얼마")은 구간을 좁혀서 타고, 쿼리가 없는 요청("핵심만 30초",
"재밌는 부분")은 기존 rank 로 바로 간다. **두 경로가 반드시 공존한다** — 후자가 곧 기존
크리에이터 경로다.

분기는 **LLM 1회**로 한다 **[미결 — 규칙 기반과 비교]**. 결과를 `runs.route` 에 남겨 어느 경로가
얼마나 쓰였는지 본다.

**구간 좁히기도 이 호출이 한다(2026-09-09 변경).** 원래는 임베딩 유사도 top-k 였는데 실측에서
안 듣는 것으로 드러났다 — "개발 직군의 기술 스택은?" 질문에서 답이 담긴 구간이 44개 중 **29위**였고,
1위는 질문의 "기술" 과 글자가 겹치는 "기술 문화" 였다. 짧은 한국어 구간 설명을 임베딩하면 구체적
기술 내용과 일반적 조직 얘기가 갈리지 않는다. 측정과 근거는 `update_plan.md` M5 실측 절에 있다.

그래서 이 호출에 **구간 설명 목록을 함께 넣고 번호로 고르게** 한다. 셋이 맞아떨어진다:
설명 44줄은 약 2천 토큰이라 싸고(비싼 건 대사다), 기준 rank 경로가 이미 같은 방식으로 잘 되고 있고,
라우팅이 이미 LLM 을 한 번 쓰고 있어서 **호출 수가 늘지 않는다**(답변당 5회 유지).

부수 효과: **"이 영상엔 그 얘기가 없다" 를 여기서 안다** — 모델이 빈 배열을 주면 그 뜻이다.
옛 검출기(`SHORTS_RETRIEVAL_MIN_SIM`)는 유사도가 0.50~0.755 좁은 띠에 몰려 한 번도 아무것도
거르지 못했다. 임베딩은 **다른 편 안내**에만 남는다 — 거기는 영상끼리의 상대 비교라 뜻이 있다.

⚠️ 한 호출에 두 일을 시키면 둘 다 조금 나빠질 수 있다. `stage_calls.params` 에 route·rule·선택을
모두 남겨 붙여 보고 잰다.
...
- ~~실패 → 최고점 part 하나로 축소해 1회 재시도~~ → **best-of-3 로 대체됐다(2026-09-06).**
  rank 가 후보 3개(single·combo·tight)를 한 번에 내고 judge 가 **병렬로** 판정해 고른다. judge 가
  텍스트를 보기 때문에 렌더 없이 셋을 판정하고 이긴 것만 렌더한다 — 순차 재시도보다 빠르고,
  두 번째 시도가 첫 번째보다 낫다는 보장이 없다는 문제도 사라진다.
- 셋 다 불합격이면 그중 최고점을 REVIEW 로 올리고 `clip_reviews(reviewer='llm', verdict='NG')` 를 남긴다.
...
## 13. 결정 사항 — 2026-09-06 확정

**M0 에서 전부 권고대로 결정했다**(update_plan D1). 반대할 새 정보가 없었고, 각 권고에 근거가 붙어
있었다. 뒤집으려면 이 표의 "결정" 열을 고치고 update_plan.md §6 에 이유를 적는다. θ·30초 예산처럼
"설정값" 으로 둔 것은 값이 바뀌어도 결정이 바뀐 게 아니다. 이 문서가 정하지 않은 결정 D2~D11 은
update_plan.md §6.

| | 선택지 | 결정 (2026-09-06) | 근거 |
|---|---|---|---|
| Flow A / B 범위 | A 집중 · B 최소 / 둘 다 | **A 집중** | B 는 기존 스튜디오 + 버튼 몇 개로 80% 있다. 새 힘은 A 로 |
| 시청자 면 접근 | 완전 공개 / 링크 아는 사람만 | **완전 공개(익명 쿠키)** | 데모에 로그인이 끼면 흐름이 끊긴다. 공개 쓰기는 레이트리밋으로. 🔴 2026-09-06 email 로그인을 검토했다가 물렀다 — 검증 없는 email 은 쿠키보다 위조가 쉬워 중복 방지가 **더 약해지고**, 첫 관문의 마찰이 질문 유입을 직접 깎는다. 알림이 필요해지면 질문을 남긴 뒤 선택적으로 받는다 |
| 묶기 임계값 θ | 0.80 ~ 0.90 | **0.85 확정** (2026-09-06 측정) | `sm answers eval-cluster` 로 0.60·0.70·0.75·0.85·0.92 를 실측했다. 0.60~0.85 구간에서 **잘못 붙은 것이 하나도 없고**(무관한 "강아지 키우시나요?" 도 계속 단독), 0.92 는 23개 질문이 17개 클러스터로 흩어져 사실상 전부 단독이 된다. 안전 구간이 넓어 θ 는 민감한 변수가 아니었다 — 묶기 품질의 대부분은 LLM 의 대표 문장 짓기가 결정한다. 그중 **높은 쪽**을 택한 이유는 실패 비용의 비대칭이다: 잘못 붙으면 크리에이터는 대표 문장만 보므로 못 알아채고, 안 붙으면 단독 클러스터로 목록에 보여 눈에 띈다 |
