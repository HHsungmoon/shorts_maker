...
- **잡은 워커 1개.** 동시 1건은 정책이 아니라 구조다 — 영상 처리와 STT 가 겹치면 API 가 죽는다.
  🔴 이미 도는 잡이 있으면 새 잡은 **409 로 거절**한다(2026-09-16, `jobs.Busy` → `http/deps.submit`).
  쌓아 두면 여럿이 각자 누른 것이 조용한 대기열이 되고 Gemini 할당량이 그만큼 탄다. 검사와 등록은
  **같은 락 안**이다 — 밖에서 `active()` 로 보고 넣으면 동시에 누른 둘이 둘 다 통과한다.
  화면은 `/api/status` 의 `activeJob` 을 5초마다 읽어 "다른 창에서 실행 중" 을 알린다
- **중립/주관을 섞지 않는다.** Segment(사실)는 캐시해 재사용하고 Run(기준)만 갈아끼운다.
  세그먼트 해설/요약에 특정 기준을 넣는 순간 그건 중립 자산이 아니다
- **프롬프트는 네 층이다** — 규칙(코드) · 관리자 기준 · 영상 개요 · 이번 요청(`standards.py` 머리 주석).
  🔴 **관리자 기준은 구간 분할과 답 구간 찾기(routing)에 넣지 않는다.** 앞의 것은 중립 자산이고 뒤의 것은
  사실 판단이다. 판정에는 **점수에만** 넣는다. `tests/test_standards.py` 가 이 경계를 지킨다.
  규칙(1층)은 스튜디오에서 보이기만 하고 고칠 수 없다 — 템플릿을 고치면 그 화면이 원문을 그대로 따라온다
...
- 시청자용 공개 API 는 `http/watch.py` 라우터다. **인증이 없다 — 인터넷에 그대로 열린다.** 규칙 셋:
  ...
  ② 🔴 **외부 호출은 질문 등록의 숏폼 추천 한 곳, 임베딩 1회뿐이다**(2026-09-15, update_plan D13). LLM 은 0회.
     울타리 여섯: 질문을 먼저 커밋 · 비교할 발행 숏폼이 없으면 호출 0회 · 재시도 없음 · 제한 시간 ·
     공개 경로 전용 분당 상한 · 실패하면 추천만 조용히 뺀다(`answers/suggest.py`). 묶기는 여전히 [집계]가 한다
...
- **역할이 둘이다**(2026-09-16). `SHORTS_ADMIN_PASSWORD` 는 전부, `SHORTS_READONLY_PASSWORD` 는 **읽기만**.
  ...
  화면의 버튼 잠금은 **안내**일 뿐이고 울타리는 서버다. 🔴 화면에서 막을 때 **버튼을 비활성화하지
  않는다** — 눌리지 않는 버튼은 이유를 말할 기회가 없어 "고장난 화면" 으로 읽힌다. 눌리게 두고
  `AuthContext.refuse()` 한 곳으로 모아 **화면 위 토스트**로 알린다(`components/DeniedToast`).
  서버의 403 도 같은 토스트로 들어온다 — 거절 경로가 하나여야 어떤 버튼은 배너, 어떤 버튼은
  무반응이 되지 않는다. 모달로 만들었다가 물렀다(2026-09-17): 한 줄 안내에 화면을 덮고 닫기를
  강요하면 구경하는 흐름이 매번 끊긴다. 본문 배너로도 돌아가지 않는다 — 아래쪽 버튼을 누른
  사람 눈에 안 들어온다. 🔴 보기 전용 비밀번호만 넣으면 인증이 켜지지
  않는다 — 두 비밀번호가 같아도 기동을 거부한다(`server.check_binding`)
- 🔴 **보기 전용 비밀번호는 공개된다.** `/auth/me` 가 무인증으로 내주고 첫 화면과 로그인 화면이 사람에게
  그대로 보여준다(2026-09-16 — 해커톤 방문자가 물어볼 사람 없이 들어오게 하려는 것이다). 그러니
  `SHORTS_READONLY_PASSWORD` 에는 **다른 곳에서 쓰는 값을 재활용하지 않는다.** 관리자 비밀번호는 어떤
  응답에도 싣지 않는다 — `test_api_auth.py` 가 응답 본문을 통째로 훑어 그 값이 없음을 확인한다.
  화면에 상수로 박지 않는 이유는 `.env` 를 바꾼 순간 화면이 옛 값을 보여주고, 그걸 그대로 친 방문자가
  로그인 실패 잠금(5회/60초)에 걸리기 때문이다
- 로그인 실패 잠금은 **IP 별**이다(2026-09-16, 5회/60초 창). 🔴 전역 카운터였을 때는 **남 다섯 명의
  오타가 나를 잠갔다** — 비밀번호를 화면에 공개한 순간 그건 실사용 경로다. IP 를 바꿔 가며 두드리는
  쪽은 전역 백스톱(`GLOBAL_MAX_FAILURES`)이 받는다. IP 는 nginx 가 덮어쓰는 `X-Real-IP` 에서 온다
  (`answers/viewers.client_ip`) — `X-Forwarded-For` 는 클라이언트가 앞을 위조할 수 있어 쓰지 않는다.
  붙여넣기에 딸려 온 앞뒤 공백은 실패로 세지 않는다
